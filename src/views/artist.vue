<template>
  <div class="info_box">
    ...
    <el-dialog title="添加艺术家" :visible.sync="artistBox" width="500px">
      <el-form ref="artistform" label-width="100px">
        <el-form-item label="艺术家名称">
          <el-input v-model="name" placeholder="输入艺术家名称"> </el-input>
        </el-form-item>
        <el-form-item label="艺术家封面" prop="url">
          <el-upload
            action=""
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="artistBox = false">取 消</el-button>
  ...
  methods: {
    ...
    async saveArtist() {
      if (this.name.length < 1) {
        this.$message.warning("艺术家名称不能为空");
        return;
      }
      let res = await this.$http.get("/manage/saveartist", {
        name: this.name,
        face: this.face,
        desc: this.desc,
        id: this.id,
